...
describe("#ValidationManager", () => {
  ...
  it("test sanity: reject unknown rule", async () => {
    expect(await testUserOp("<unknown-rule>").catch((e) => e.message)).to.match(/unknown rule/);
  });
  it("should fail with bad opcode in ctr", async () => {
    expect(
      await testUserOp("", undefined, opcodeFactory.interface.encodeFunctionData("create", ["coinbase"])).catch(
        (e) => e.message
      )
    ).to.match(/factory uses banned opcode: COINBASE/);
  });
  it("should fail with bad opcode in paymaster", async () => {
    expect(await testUserOp("", "coinbase", undefined).catch((e) => e.message)).to.match(
      /paymaster uses banned opcode: COINBASE/
    );
  });
  it("should fail with bad opcode in validation", async () => {
    expect(await testUserOp("blockhash").catch((e) => e.message)).to.match(/account uses banned opcode: BLOCKHASH/);
  });
  it("should fail if creating too many", async () => {
    expect(await testUserOp("create2").catch((e) => e.message)).to.match(/account uses banned opcode: CREATE2/);
  });
  ...
  describe("access allowance (existing wallet)", () => {
    it("account fails to read allowance of other address (even if account is token owner)", async () => {
      expect(await testExistingUserOp("allowance-self-1").catch((e) => e.message)).to.match(
        /account has forbidden read/
      );
    });
  ...
  describe("access struct (existing wallet)", () => {
    ...
    it("should fail to access other address struct data", async () => {
      expect(await testExistingUserOp("struct-1").catch((e) => e.message)).match(/account has forbidden read/);
    });
  });
  ...
  it("should fail if validation recursively calls handleOps", async () => {
    ...
    expect(await vm.validateUserOp(op).catch((e) => e.message)).to.match(/illegal call into EntryPoint/);
  });

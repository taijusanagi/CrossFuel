...
export class ReputationManager {
  constructor(readonly params: ReputationParams, readonly minStake: BigNumber, readonly minUnstakeDelay: number) {}
  ...
  checkStake(title: "account" | "paymaster" | "aggregator" | "deployer", info?: StakeInfo): void {
    ...
    requireCond(
      ...
      `${title} ${info.addr} unstake delay ${tostr(info.unstakeDelaySec)} is too low (min=${tostr(
        this.minUnstakeDelay
      )})`,
      ValidationErrors.InsufficientStake
    );
  }

...
export class BundleManager {
  ...
  async sendBundle(
    ...
    try {
      ...
      if (this.conditionalRpc) {
        ...
        ret = await this.provider.send("eth_sendRawTransactionConditional", [signedTx, { knownAccounts: storageMap }]);
  ...
  async createBundle(): Promise<[UserOperation[], StorageMap]> {
    ...
    for (const entry of entries) {
      ...
      if (
        paymaster != null &&
        (paymasterStatus === ReputationStatus.THROTTLED ?? (stakedEntityCount[paymaster] ?? 0) > 1)
      ) {
  ...
  async getUserOpHashes(userOps: UserOperation[]): Promise<string[]> {
    const { userOpHashes } = await runContractScript(this.entryPoint.provider, new GetUserOpHashes__factory(), [
